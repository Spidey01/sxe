...
public class SettingsArgs implements Settings {
    ...
    public SettingsArgs(String[] args) {
        for (String arg : args) {
            int sep = arg.lastIndexOf("=");
            mArgs.put(arg.substring(0, sep), arg.substring(sep+1));
        }
    }
    ...
    @Override
    public String[] keys() {
        Set<String> length = mArgs.keySet();
    ...
    public Settings setBoolean(String key, boolean value) {
        notifyListeners(key);
        mArgs.put(key, Boolean.toString(value));
        return this;
    }
    ...
    public Settings setFloat(String key, float value) {
        notifyListeners(key);
        mArgs.put(key, Float.toString(value));
        return this;
    }
    ...
    public Settings setInt(String key, int value) {
        notifyListeners(key);
        mArgs.put(key, Integer.toString(value));
        return this;
    }
    ...
    public Settings setLong(String key, long value) {
        notifyListeners(key);
        mArgs.put(key, Long.toString(value));
        return this;
    }
