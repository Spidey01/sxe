/*-
 * Copyright (c) 2014-current, Terry Mathew Poulin <BigBoss1964@gmail.com>
 *
 * This software is provided 'as-is', without any express or implied warranty.
 * In no event will the authors be held liable for any damages arising from the
 * use of this software.
 *
 * Permission is granted to anyone to use this software for any purpose,
 * including commercial applications, and to alter it and redistribute it
 * freely, subject to the following restrictions:
 *
 *	1. The origin of this software must not be misrepresented; you must
 *	   not claim that you wrote the original software. If you use this
 *	   software in a product, an acknowledgment in the product
 *	   documentation would be appreciated but is not required.
 *
 *	2. Altered source versions must be plainly marked as such, and must
 *	   not be misrepresented as being the original software.
 *
 *	3. This notice may not be removed or altered from any source
 *	   distribution.
 */
package com.spidey01.sxe.core.config;

import org.junit.*;

import com.spidey01.sxe.core.logging.Log;
import com.spidey01.sxe.core.testing.UnitTest;
import com.spidey01.sxe.core.testing.TestResources;

import java.io.File;
import java.io.IOException;


public class SettingsExpanderTest
    extends UnitTest
{
    private static final String TAG = "SettingsExpanderTest";

    @BeforeClass
    public static void setup() {
        UnitTest.setup();
    }


    @Test
    public void expansion() {
        Log.test(TAG, "expansion");
        Settings test = new SettingsFile(new File(TestResources.directory, TestResources.settingsFileName));
        SettingsExpander e = new SettingsExpander(test);

        Log.test(TAG, "ONE");
        test.setString("foo", "bar");
        // Assert.assertEquals("bar", test.getString("foo"));
        Log.test(TAG, "getString in test");
        Assert.assertEquals("bar", e.expand("foo"));


        Log.test(TAG, "TWO");
        test.setString("bar", "ham");
        Assert.assertEquals("ham", e.expand("bar"));


        Log.test(TAG, "THREE");
        test.setString("x", "foo");
        test.setString("y", "bar");
        test.setString("xy", "{x}+{y}");
        Assert.assertEquals("Simple expansion.",
                "foo+bar", e.expand("xy"));


        Log.test(TAG, "FOUR");
        test.setString("first", "{bar}");
        test.setString("last", "eggs");
        test.setString("quux", "{first}, spam, and {last}");
        Assert.assertEquals("Nested expansion.",
                // "ham, spam, and eggs",   // recursive
                "{bar}, spam, and eggs", // non-recursive
                e.expand("quux"));
    }

}

