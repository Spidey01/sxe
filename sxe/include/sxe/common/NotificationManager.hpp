#ifndef SXE_COMMON_NOTIFICATIONMANAGER__HPP
#define SXE_COMMON_NOTIFICATIONMANAGER__HPP
/*-
    ...
    class NotificationManager
    {
      public:
        ...
        size_type subscribe(Receiver_T rx)
        {
            ...
            sxe::logging::Log::log(mLevel, mTag, "subscribe(): rx: " + asString(rx));
        ...
        size_type subscribe(Receiver_T rx, const string_type& key)
        {
            ...
            sxe::logging::Log::log(mLevel, mTag, "subscribe(): rx: " + asString(rx) + " key: " + key);
        ...
        {
            sxe::logging::Log::log(mLevel, mTag, "unsubscribe(): id: " + std::to_string(id));
        ...
        {
            sxe::logging::Log::log(mLevel, mTag,
        ...
        {
            sxe::logging::Log::log(mLevel, mTag, "notifyListeners(): " + toKey(message));

            lock_type guard(mMutex);

            sxe::logging::Log::log(mLevel, mTag, "notifyListeners(): all messages subscribers: " + toKey(message));
            for (auto rx : notificationReceivers()) {
                invoke(rx, message);
            }

            sxe::logging::Log::log(mLevel, mTag, "notifyListeners(): specific message subscribers: " + toKey(message));
        ...
        NotificationManager()
            ...
            , mLevel(sxe::logging::Log::TEST)
            , mNotificationReceivers()
            , mSubscribers()
        {
            sxe::logging::Log::log(mLevel, mTag, "NotificationManager<>()");
        }
      ...
      protected:
        ...
        {
            sxe::logging::Log::log(mLevel, mTag, "invoke(): rx: " + asString(rx) + " message: " + toKey(message));
            rx(message);
        }
        ...
        string_type toKey(const Key_T& msg) const
        {
            if constexpr(std::is_convertible<Key_T, std::string>::value) {
                string_type k = msg;
                sxe::logging::Log::log(mLevel, mTag, "toKey(msg): operator string(): " + k);
                return k;
            } else {
                Hash h;
                size_t k = h(msg);
                sxe::logging::Log::log(mLevel, mTag, "toKey(msg): Hash{}(): " + std::to_string(k));
...
#endif // SXE_COMMON_NOTIFICATIONMANAGER__HPP
